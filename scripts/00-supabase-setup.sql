...
CREATE TABLE emails (
    ...
    body TEXT,
    html_body TEXT,
    email_type VARCHAR(20) DEFAULT 'outbound',
...
-- Nommer explicitement les contraintes de clés étrangères
...
ALTER TABLE emails 
ADD CONSTRAINT emails_collaborator_id_fkey 
...
-- Politiques RLS basiques (à personnaliser selon vos besoins)
CREATE POLICY "Users can view own data" ON contacts
    FOR SELECT USING (auth.uid()::text = assigned_to::text);

CREATE POLICY "Users can view own notifications" ON notifications
    FOR SELECT USING (auth.uid()::text = collaborator_id::text);
